...
void cleanup_thrust();
void voxelize(const voxinfo & v, float* triangle_data, unsigned int* vtable, bool useThrustPath, bool morton_code, bool silent);
...
int main(int argc, char* argv[]) {
	...
	auto bbox_min = themesh->bbox.min;
	auto bbox_max = themesh->bbox.max;
	if (forceBound) {
		bbox_min[0] = -0.5;
		bbox_min[1] = -0.5;
		bbox_min[2] = -0.5;
	...
	AABox<glm::vec3> bbox_mesh_cubed = createMeshBBCube<glm::vec3>(AABox<glm::vec3>(trimesh_to_glm(bbox_min), trimesh_to_glm(bbox_max)));
	...
	if (cuda_ok && !forceCPU) { 
		...
		if (solidVoxelization){
			...
			voxelize(voxelization_info, device_triangles, vtable, useThrustPath, (outputformat == OutputFormat::output_morton), silent);
		}
	} else { 
	...
	if (outputformat == OutputFormat::output_morton){
		...
		write_binvox(vtable, voxelization_info, filename, silent);
	}
